    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                # >>> 新增：LLaDA + projector-only（从 model_path 取 mm_projector.bin）
                from llava.model.language_model.llava_llada import LlavaLLaDAConfig, LlavaLLaDAModelLM
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False, trust_remote_code=True)
                cfg_pretrained = LlavaLLaDAConfig.from_pretrained(model_path, trust_remote_code=True)
                model = LlavaLLaDAModelLM.from_pretrained(
                    model_base, low_cpu_mem_usage=True, config=cfg_pretrained,
                    attn_implementation=attn_implementation, **kwargs
                )
                # 和原分支保持一致，尝试加载 projector
                ...
                if os.path.exists(mm_projector_path):
                    ...
                    rank0_print(f"[Info] {mm_projector_path} not found, skip projector-only loading.")            
            elif (
                ...
                if customized_config is None:
                    llava_cfg = LlavaConfig.from_pretrained(model_path, trust_remote_code=True)
