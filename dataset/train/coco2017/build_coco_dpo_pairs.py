...
# --- 路径配置 ---
COCO_ROOT = "/hpc2ssd/JH_DATA/spooler/yuxuanzhao/lijungang/wujie/LLaDA-V/dataset/coco2017"
SPLIT = "val2017"
SAVE_DIR = os.path.join(COCO_ROOT, "llava_multi")
OUT_DPO = os.path.join(SAVE_DIR, f"coco_{SPLIT}_dpo_final_mixed_v2_en.jsonl") # 最终混合模式版
...
def build_rejected_multi_cat(catname2boxes_orig, target_cat, target_idx, mode):
    ...
    if mode == "drop":
        ...
        if not target_boxes: del catname2boxes_rej[target_cat]
...
for img_id, cat2boxes in tqdm(img2cat2boxes.items(), desc="Processing images"):
    ...
    if small_objects_pool:
        ...
        if rejected_dict:
            ...
            item = {"image": image_rel, "prompt": prompt, "chosen": ans_from_multi_cat(chosen_dict), "rejected": ans_from_multi_cat(rejected_dict), "meta": {"img_id": img_id, "type": "multi_object_small_focus", **neg_meta}}
    ...
    if FALLBACK_TO_SINGLE_CAT_DPO and not dpo_pair_generated_for_image_multi:
        for cat, boxes in cat2boxes.items():
            ...
            # ############ BUG FIX ############
            # The `pair_is_valid` function expects a list of boxes (list of lists),
            # not a flattened list of coordinates.
            ...
            # ###############################
                ...
                item = {"image": image_rel, "prompt": prompt, "chosen": ans_from_single_cat(boxes, cat), "rejected": ans_from_single_cat(rej_boxes, cat), "meta": {"img_id": img_id, "type": "single_object_fallback", "neg_mode": "jitter_any"}}
                final_items.append(item)
