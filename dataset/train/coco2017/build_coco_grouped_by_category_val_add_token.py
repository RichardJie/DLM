...
OUT_JSON = os.path.join(SAVE_DIR, f"coco_{SPLIT}_sft_add_token.json") # 最终版v2文件名
...
# ====== 3. 工具函数与数据加载 ======
# ==============================================================================
def format_answer_with_special_tokens(count_str, bbox_dict_str):
    """
    用特殊 token 包裹答案的不同部分
    <COUNT>数量信息</COUNT><COORD>坐标信息</COORD>
    """
    return f"<COUNT>{count_str}</COUNT><COORD>{bbox_dict_str}</COORD>"
...
CAND_ANN = [
    os.path.join(ANNO_DIR_1, f"instances_{SPLIT}.json"),
    os.path.join(ANNO_DIR_2, f"instances_{SPLIT}.json"),
]
ANN_FILE = next((p for p in CAND_ANN if os.path.exists(p)), None)
assert ANN_FILE, f"找不到 instances_{SPLIT}.json，请检查 {ANNO_DIR_1} 或 {ANNO_DIR_2}"
...
for img_id, anns in tqdm(img2anns.items(), desc="Processing images"):
    ...
    if len(present_cats) > 1:
        cats_list_str = ", ".join(sorted(list(present_cats)))
        ...
        # 核心修改：添加特殊 token
        count_part = f"counts: {count_breakdown}"
        bbox_part = f"bbox_dict: {json.dumps(answer_dict, ensure_ascii=False)}"
        answer = format_answer_with_special_tokens(count_part, bbox_part)
